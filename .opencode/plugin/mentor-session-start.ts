...
async function searchBerry(query: string, limit = 10): Promise<BerrySearchResult | null> {
  try {
...
export const MentorSessionPlugin: Plugin = async () => {
  ...
  async function getSessionContext(): Promise<string> {
    ...
    const results = await searchBerry("session-summary mentor staff assignment", 10)
...
Call this tool at the END of a mentor session to persist key outcomes, decisions, and any new assignments given.`,
        args: {
          summary: tool.schema
            .string()
            .describe("Summary of the session including key topics, decisions, and outcomes"),
        ...
        async execute(args) {
          ...
          if (args.assignments && args.assignments.length > 0) {
            for (const assignment of args.assignments) {
              try {
                const assignmentResponse = await fetch(`${BERRY_API_URL}/remember`, {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({
                    content: assignment,
                ...
                results.push(`Failed to store assignment: ${assignment.substring(0, 50)}...`)
              }
            }
          }
